...
struct XV11Packet
{	
	uint32_t m_timestamp_us;//timestamp in microseconds
	uint8_t m_angle_quad;	//0-89 for readings 0-4 356-359
	uint16_t m_speed64;		//divide by 64 for speed in rpm
	uint32_t m_readings[4];	//distance, error, flags and signal strength 
};
...
private:
	...
	uint16_t decode_u16(const uint8_t *data) const;
private:
	/* Hardware */
